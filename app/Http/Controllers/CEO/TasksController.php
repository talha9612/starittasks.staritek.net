...
class TasksController extends Controller
{
    public function index()
    {
        ...
        // Fetch all users in the same company with role 3 (team members)
        $users = User::where('user_type', $userType)
            ->where('role', 3)
            ->get();
        // Count managers (role 2) in the same company
        ...
        for ($i = 0; $i < sizeof($userIds); $i++) {
            $project = Project::with('head', 'createproject', 'projectcatagory', 'assign_project.GetUsers')
                ->where('create_project', $userIds[$i])
                ->get()
                ->toArray(); // Convert to array if not already  
            $projects = array_merge($projects, $project);
        }

        $projects = collect($projects);
        $tasks = Task::with('GetUsers')->whereIn('user_id', $userIds)->get();
        //dd( $tasks);
        return view('ceo.taskhome', compact('projects', 'tasks'));
    }
