...
class ProfileController extends Controller
{
    public function index(){
        ...
        // Get Admin ID
        $adminuser = User::where('id',Auth::user()->user_type)->first();
        $company = CompanySetting::where('user_id',$adminuser->user_type)->orwhere('user_id',$adminuser->id)->first();
        $manager_ids = [];
        $departments = [];
        $skills = [];
        $mana = User::where('user_type',Auth::user()->user_type)->first();
        $manager = User::where('id',$mana->user_type)->first();
        $managers = User::where('user_type',$manager->user_type)->get();
        // $getRecords = User::where('')
        ...
        return view('user.profile',compact('user','company','departments','skills'));
    }
